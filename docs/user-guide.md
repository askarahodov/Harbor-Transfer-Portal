...
- **SOURCE** работает только с Harbor profiles, достижимыми из текущего изолированного контура, и создаёт подписанный offline bundle;
- **TARGET** работает только с Harbor profiles текущей installation, проверяет полученный bundle и импортирует его;
- переключение SOURCE/TARGET меняет рабочую роль Portal, но **не выбирает Harbor автоматически** и не меняет credentials;
- если настроено несколько Harbor profiles, operator/admin выбирает нужный profile в Export/Import до начала operation; после старта выбор фиксируется в operation;
...
## 6. Шаг 1 — выберите Harbor profile и точные артефакты

Если администратор настроил несколько Harbor profiles, сначала выберите нужный **Harbor profile**. Selector показывает только доступные profiles; disabled или не настроенный profile нельзя использовать. Смена profile до preview очищает зависимый browse/selection state, чтобы артефакты одного Harbor не смешались с другим.

Выбор выполняется только из выбранного Harbor profile текущей installation в SOURCE role. После запуска export profile фиксируется в operation и не меняется вместе с UI.

Последовательно:

1. выберите Harbor profile;
2. в верхней строке найдите и выберите Harbor project;
