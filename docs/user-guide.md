...
| `CONFLICT` | то же имя/tag/version существует с другим digest | по умолчанию блокировать; можно явно пропустить без изменения TARGET |
...
Даже если overwrite запрещён, UI предлагает безопасное действие **«Импортировать отсутствующие · существующие пропустить»**. В этом режиме CONFLICT остаётся нетронутым в TARGET, фиксируется как явно пропущенный, а NEW продолжают импортироваться.
...
Если разрешён и явно подтверждён конфликтный overwrite, рядом остаётся отдельное опасное действие **«Импортировать с подтверждённым overwrite»**. Skip и overwrite одновременно не применяются.
