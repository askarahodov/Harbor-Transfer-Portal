...
describe('import wizard store', () => {
  it('uploads a raw bundle, persists operation id and opens verified preview', async () => {
  ...
  it('blocks conflict by default and requires backend policy plus explicit overwrite confirmation', async () => {
    ...
    expect(executeSpy).toHaveBeenCalledWith(51, true)
    expect(store.step).toBe(3)
    store.stopPolling()
  })

  it('restores active import after reload and loads receipt on partial failure', async () => {
    sessionStorage.setItem('htp.import.operation-id', '51')
    vi.spyOn(importsApi, 'getOperation')
      .mockResolvedValueOnce(operation('IMPORTING'))
