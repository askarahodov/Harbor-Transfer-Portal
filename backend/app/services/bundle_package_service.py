...
class BundlePackageService:
    ...
    def allocate_delivery_id(self, now: datetime | None = None) -> str:
        ...
        random_part = "".join(secrets.choice(_SAFE_TOKEN_ALPHABET) for _ in range(12))
    ...
    def build_bundle(
        ...
        *,
        ...
        if not artifacts:
            raise BundlePackageError("bundle_artifacts_empty", "Bundle должен содержать артефакты")
        if source.contour != "SOURCE":
            raise BundlePackageError("bundle_source_invalid", "Bundle source contour должен быть SOURCE")
        ...
        if _DELIVERY_ID_RE.fullmatch(identifier) is None:
            raise BundlePackageError("bundle_delivery_id_invalid", "delivery_id имеет неверный формат")
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="bundle-build-", dir=self.temp_root) as name:
                workspace = Path(name)
                os.chmod(workspace, 0o700)
                staged_inputs = self._stage_payloads(workspace, artifacts)
                checksum_entries = self._checksum_payload_files(workspace, staged_inputs)
                descriptors = self._build_descriptors(staged_inputs, checksum_entries)
                try:
                    manifest = BundleManifest(
                        schema_version="1.0",
                ...
                (workspace / "manifest.sig").write_bytes(signature)
                (workspace / "checksums.sha256").write_bytes(
                    self._serialize_checksums(checksum_entries)
                )

                temporary_archive = workspace / archive_name
                self._write_deterministic_archive(workspace, temporary_archive)
            ...
            self._cleanup_partial_publish(final_archive, final_sidecar)
            raise
    ...
    def verify_bundle(
        ...
        *,
        ...
        keys = self._load_trusted_public_keys()
        ...
        *,
        ...
        try:
            with tarfile.open(archive, mode="r:gz") as bundle:
                ...
                manifest_bytes = self._read_small_member(bundle, "manifest.json", inspection)
                signature = self._read_small_member(bundle, "manifest.sig", inspection)
                ...
                fingerprint = self._verify_signature(manifest_bytes, signature, trusted_keys)
        ...
        if extract_to is not None:
            extracted = self._extract_verified_archive(archive, extract_to)
    ...
    def _stage_payloads(
        ...
        normalized: list[PackageArtifactInput] = []
        ...
        for artifact in artifacts:
            ...
            if isinstance(artifact, ContainerImagePackageInput):
                ...
                if not source.is_file() or source.is_symlink() or source.suffix != ".tgz":
                    raise BundlePackageError(
    ...
    def _checksum_payload_files(
        ...
        for artifact in artifacts:
            ...
            if staged.is_file():
                ...
                candidates = tuple(sorted(path for path in staged.rglob("*") if path.is_file()))
            if not candidates:
                raise BundlePackageError("bundle_payload_empty", "Payload не содержит файлов")
            for file_path in candidates:
                relative = file_path.relative_to(workspace).as_posix()
                self._validate_member_name(relative)
                entries[relative] = (self._sha256_file(file_path), file_path.stat().st_size)
    ...
    def _write_deterministic_archive(self, workspace: Path, archive_path: Path) -> None:
        entries = sorted(
            ...
            if path != archive_path and not path.is_symlink()
        )
    ...
    def _inspect_archive(self, archive: tarfile.TarFile, archive_size: int) -> _ArchiveInspection:
        ...
        return _ArchiveInspection(members=members, file_members=file_members, dir_members=dirs)
    ...
    def _read_small_member(
        ...
        if member is None:
            raise BundlePackageError("bundle_required_file_invalid", f"{path} должен быть файлом")
        if member.size > self.settings.bundle_max_metadata_bytes:
            raise BundlePackageError("bundle_metadata_too_large", f"{path} превышает допустимый размер")
        stream = archive.extractfile(member)
        if stream is None:
            raise BundlePackageError("bundle_archive_invalid", f"Не удалось прочитать {path}")
        return stream.read(self.settings.bundle_max_metadata_bytes + 1)
    ...
    def _parse_canonical_manifest(self, manifest_bytes: bytes) -> dict[str, Any]:
        try:
            ...
            raise BundlePackageError("bundle_manifest_invalid", "manifest.json содержит неверный JSON") from exc
        if not isinstance(payload, dict):
            raise BundlePackageError("bundle_manifest_invalid", "manifest.json должен быть JSON object")
    ...
    def _drop_nulls(value: Any) -> Any:
        if isinstance(value, dict):
            return {key: BundlePackageService._drop_nulls(item) for key, item in value.items() if item is not None}
    ...
    def _load_manifest_schema() -> dict[str, Any]:
        schema_file = resources.files("app.domain").joinpath("manifest-v1.schema.json")
        try:
        ...
        if not isinstance(payload, dict):
            raise BundlePackageError("bundle_schema_runtime_invalid", "Runtime JSON Schema некорректна")
        return payload
    ...
    def _verify_signature(
        ...
        if len(signature) != 64:
            raise BundlePackageError("bundle_signature_invalid", "Ed25519 signature имеет неверный размер")
    ...
    def _parse_checksums(self, payload: bytes) -> dict[str, str]:
        ...
        for line in text.splitlines():
            if not line:
                raise BundlePackageError("bundle_checksums_invalid", "Пустая checksum-строка запрещена")
            if len(line) < 67 or line[64:66] != "  ":
                raise BundlePackageError("bundle_checksums_invalid", "Неверный формат checksum-строки")
            digest = line[:64]
            path = line[66:]
            if _SHA256_RE.fullmatch(digest) is None:
                raise BundlePackageError("bundle_checksums_invalid", "Неверный SHA-256 в checksums")
            self._validate_member_name(path)
            if path in _CRITICAL_FILES or path in entries:
                raise BundlePackageError("bundle_checksums_invalid", "Duplicate/critical checksum path")
            entries[path] = digest
        if not entries:
            raise BundlePackageError("bundle_checksums_invalid", "checksums.sha256 не содержит payload")
        return entries
    ...
    def _verify_descriptor_metadata(
        ...
        checksum_sizes = {
            path: inspection.file_members[path].size
            for path in checksums
        }
        combined = {
            path: (checksums[path], checksum_sizes[path])
            for path in checksums
        }
        for artifact in manifest.artifacts:
            stats = self._payload_stats(artifact.payload_path, combined)
            if stats.sha256 != artifact.payload_sha256 or stats.size != artifact.payload_size:
                raise BundlePackageError(
    ...
    def _payload_stats(
        ...
        if not matches:
            raise BundlePackageError("bundle_payload_missing", f"Payload отсутствует: {payload_path}")
        ...
        canonical = "".join(f"{digest}  {path}\n" for path, digest, _size in matches).encode()
        return _PayloadStats(sha256=hashlib.sha256(canonical).hexdigest(), size=total_size)

    def _extract_verified_archive(self, archive: Path, destination: Path) -> Path:
        ...
        temporary = Path(tempfile.mkdtemp(prefix="bundle-extract-", dir=target.parent))
        os.chmod(temporary, 0o700)
        try:
            with tarfile.open(archive, mode="r:gz") as bundle:
                inspection = self._inspect_archive(bundle, archive.stat().st_size)
                for member in inspection.members:
                    ...
                    if stream is None:
                        raise BundlePackageError("bundle_archive_invalid", f"Не удалось извлечь {name}")
    ...
    def _atomic_publish(
        ...
        sidecar_tmp = self.outgoing_root / f".{final_sidecar.name}.{secrets.token_hex(6)}.tmp"
        try:
    ...
    def _verify_sidecar(self, sidecar_path: Path, archive: Path, actual_sha: str) -> None:
        sidecar = sidecar_path.resolve()
        if not sidecar.is_file() or sidecar.is_symlink():
            raise BundlePackageError("bundle_sidecar_invalid", "Bundle sidecar не найден")
        if sidecar.stat().st_size > self.settings.bundle_max_metadata_bytes:
            raise BundlePackageError("bundle_sidecar_invalid", "Bundle sidecar слишком большой")
        try:
            text = sidecar.read_text(encoding="utf-8")
        except (OSError, UnicodeDecodeError) as exc:
            raise BundlePackageError("bundle_sidecar_invalid", "Bundle sidecar не читается") from exc
    ...
    def _load_trusted_public_keys(self) -> tuple[Ed25519PublicKey, ...]:
        ...
        key_files = sorted(path for path in directory.glob("*.pem") if path.is_file())
        ...
        for path in key_files:
            if path.is_symlink():
                raise BundlePackageError("bundle_trusted_key_invalid", "Symlink key file запрещён")
            try:
    ...
    def _validate_private_key_file(path: Path) -> None:
        ...
        if not stat.S_ISREG(info.st_mode) or path.is_symlink():
            raise BundlePackageError("bundle_signing_key_invalid", "Signing key должен быть обычным файлом")
    ...
    def _validate_payload_destination(self, artifact: PackageArtifactInput) -> PurePosixPath:
        path = PurePosixPath(artifact.payload_path)
        self._validate_member_name(path.as_posix())
        if len(path.parts) < 2:
            raise BundlePackageError("bundle_payload_path_invalid", "Payload path слишком короткий")
        if isinstance(artifact, ContainerImagePackageInput):
    ...
    def _validate_member_name(self, name: str) -> None:
        ...
        if (
            ...
            raise BundlePackageError("bundle_archive_unsafe", f"Небезопасный bundle path: {name!r}")
    ...
    def _check_payload_limits(self, count: int, size: int) -> None:
        if count > self.settings.bundle_max_member_count:
            raise BundlePackageError("bundle_member_limit_exceeded", "Payload содержит слишком много файлов")
    ...
    def _serialize_checksums(entries: dict[str, tuple[str, int]]) -> bytes:
        return "".join(f"{digest}  {path}\n" for path, (digest, _size) in entries.items()).encode()
    ...
    @staticmethod
    def _cleanup_partial_publish(archive: Path, sidecar: Path) -> None:
