    ...
    async def prepare_retry(
        ...
        *,
        ...
        with self.session_factory() as session:
            ...
            try:
                self.orchestrator._verified_operation_binding(original_operation_id)
        ...
        with harbor_profile_boundary():
            with self.session_factory() as session:
                ...
                try:
                    current_binding = operation_binding(
                        session,
                    ...
                    raise ImportOrchestrationError(exc.code, exc.message) from exc
                if (
                    snapshot.harbor_profile_id is not None
                    and current_binding.id != snapshot.harbor_profile_id
                ):
                    raise ImportOrchestrationError(
